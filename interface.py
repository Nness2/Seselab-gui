...
def getInfo (program,i):
	...
	code = code.replace('\t','')
	code = code.replace('\n','')
	return [code,file,line,program[i][0]]
...
class Interface:
	...
	def __init__(self, code):
		...
		self._lab1.grid(column = 1, row = 1, sticky='w')
		self._lab2.grid(column = 1, row = 2, sticky='w')
		self._lab3.grid(column = 1, row = 3, sticky='w')
