...
class Interface:
	...
	def __init__(self, code):
		...
		value = StringVar() 
		value.set('')
		self._entree = Entry(self._root, text=value, width=25, font=('helvetic', 12))
		self._entree.grid(column=1, row=0, sticky='w', padx = 7, pady = 5, columnspan=2)
		self._open = Button(self._root, text="Open file", command=self.step, width = 6, font=('helvetic', 11)).grid(column=3, row=0, sticky='nesw', pady=2)
		self._load = Button(self._root, text="Load", command=self.step, width = 6, font=('helvetic', 11)).grid(column=4, row=0, sticky='nesw', pady=2)
		# self._space = Label(self._root, text='', bg=None).grid(column = 1, row = 1)
		self._lab1 = Label(self._root, text='', bg='white', width=61, anchor='w',font="TkFixedFont")
		self._lab2 = Label(self._root, text=self._infos[0], bg='yellow', width=61, anchor='w',font="TkFixedFont")
		self._lab3 = Label(self._root, text=self._infos[1], bg='white', width=61, anchor='w',font="TkFixedFont")
		...
		self._step = Button(self._root, text="Step", command=self.step, width = 6, font=('helvetic', 10)).grid(column=8, row=2, sticky='nesw', padx=2)
		self._breaK = Button(self._root, text="Break", command=self.breaK, width = 6, font=('helvetic', 10)).grid(column=8, row=4, sticky='nesw', padx=2)
		self._run = Button(self._root, text="Run", command=self.run, width = 6, font=('helvetic', 10)).grid(column=8, row=3, sticky='nesw', padx=2)
		# value = StringVar() 
		# value.set('50 ms')
		...
		self._injectFault = Label(self._root, text='Inject fault:', bg=None, width=10, font=('helvetic', 11, 'bold')).grid(column = 1, row = 11, columnspan=8, sticky='w', padx = 8)
	...
	def openRegList (self):
		...
		for j in range(4):
			for k in range(8):
				l = Button(self._root, text='r'+str(i)+': '+str(self._reg[i]), bg='white', width = 9,command=lambda x=i : self.select(x), font=('helvetic', 11))
				l.grid(column = k+1, row = j+7, sticky='we')
				self._regLst.append(l)
				i += 1
		self._regLst[0].config(bg='yellow')
		fault = Button(self._root, text="Random", command=self.injctRand, width = 6, font=('helvetic', 11)).grid(column=2, row=11, sticky='we', pady = 4)
		zero = Button(self._root, text="Zero", command=self.SetToZero, width = 6, font=('helvetic', 11)).grid(column=3 , row=11, sticky='we', pady = 4)
